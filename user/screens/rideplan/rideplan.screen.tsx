...
const RideplanScreen = () => {
  ...
  const initializeWebSocket = () => {
    ws.current = new WebSocket('ws://192.168.1.102:8080');
    ws.current.onopen = () => {
  ...
  // place select
  const handlePlaceSelect = async (placeId: string | number) => {
    try {
      const response = await axios.get(
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== 'granted') {
        Toast.show(
          `Please approve your location tracking othoerwise you can't use this app!`,
          {
            type: 'danger',
            placement: 'bottom',
          }
